...
class DependencyModel(Module): 
  ...
  def __init__(self, word_types, outputs):
    ...
    self.hiddenlayer = torch.nn.Linear(in_features = 768, out_features = 128)
    self.outputlayer = torch.nn.Linear(in_features = 128, out_features = 91)
  ...
  def forward(self, inputs):
    ...
    embedded = embedding.view(inputs.shape[0], 768)
    hidden = torch.nn.functional.relu(self.hiddenlayer(embedded))
...
def train(model, loader): 
  ...
  for idx, batch in enumerate(loader):
    ...
    predictions = model(inputs.to(torch.int64))
