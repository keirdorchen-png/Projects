...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
        ...
        rep = ['']*6
        input_state = np.zeros(6)
        
        for i in range(0,3):
            if i > len(state.stack) - 1: rep[i] = '<NULL>'
            else: rep[i] = words[state.stack[-i-1]]
        for j in range(0,3):
            if (j) > len(state.buffer) - 1: rep[j+3] = '<NULL>'
            else: rep[j+3] = words[state.buffer[-j-1]]


        for k in range(0,6):
            
            if rep[k] in self.word_vocab: 
                input_state[k] = self.word_vocab[rep[k]]
                continue


            part_of_speech = pos[words.index(rep[k])]


            if part_of_speech == 'CD': input_state[k] = 0
            elif part_of_speech == 'NNP': input_state[k] = 1
            elif part_of_speech == 'ROOT': input_state[k] = 3
            else: input_state[k] = 2
    ...
    def get_output_representation(self, output_pair):  
        ...
        output_transition = np.zeros(91)
        output_transition[self.output_labels[output_pair]] = 1
